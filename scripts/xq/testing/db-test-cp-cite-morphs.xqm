...
declare %unit:test function test:nine-entries () {
  for $r in collection("cp-cite-morphs")//record
...
declare %unit:test function test:first-start () {
  let $r := collection("cp-cite-morphs")//record
  let $sum := for $rec in $r[entry[3][starts-with(text(),"urn:cite:croala:")] and entry[5][starts-with(text(),"urn:cite:croala:latmorph.morph.")] and entry[6][starts-with(text(),"orcid.org/")]]/entry[1][starts-with(text(),"urn:cts:croala:")]
  return count($rec)
