...
with open('../csv/t-cite-modr-cts-ana2.csv') as csvfile:
    reader = csv.reader(csvfile)
    for row in reader:
        abody = URIRef(row[0])
        atargets = [ URIRef(t) for t in row[1].split()]
        annobody = BNode()
        annobody1 = BNode()
#        annobody2 = BNode()
        label = Literal(row[3])
        placeuris = [ URIRef(tt) for tt in row[2].split() ]
...
croalardffile = open("../rdf/croala-cite-ana-body3.ttl", "w")
